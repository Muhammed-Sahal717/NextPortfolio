...
import {
  ...
  pressure_frag,
  viscous_frag,
} from "./LiquidEtherShaders";

/**
 * Configuration properties for the LiquidEther WebGL fluid simulation.
 */
...
/**
 * A highly optimized WebGL-based fluid dynamics simulation component.
 * Uses Three.js and custom GLSL shaders to render an interactive, viscous fluid effect.
 * Calculates advection, divergence, poisson-pressure equations, and viscous diffusion.
 */
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    if (outputLayer) {
      ...
      if (material.uniforms.palette && material.uniforms.palette.value) {
        // Create new texture and dispose old one
        const newTex = makePaletteTexture(colors);
        const oldTex = material.uniforms.palette.value;
        material.uniforms.palette.value = newTex;
        oldTex.dispose();
      }
    }
