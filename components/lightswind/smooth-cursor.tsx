...
export function SmoothCursor({
  ...
  useEffect(() => {
    // Basic touch device check so it doesn't render on mobile
    ...
    // Fallback: also set on all interactive elements
  ...
  // Make sure it doesn't render on mobile natively
  if (typeof window !== "undefined" && !window.matchMedia("(hover: hover) and (pointer: fine)").matches) return null;
    ...
    <>
      ...
      <motion.div
        style={{
          ...
          left: cursorX,
          top: cursorY,
          translateX: "-45%", // Offset slightly to match arrow tip
          translateY: "-15%",
